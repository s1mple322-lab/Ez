-- ╔══════════════════════════════════════════════════════════╗
-- ║   🎲 BASTA BOT v8  —  FilaNum por username              ║
-- ║  Busca nuestro nombre en la GUI → obtiene FilaNum       ║
-- ║  Filtra TextBox_Fila{N} en cada columna (1 por col)     ║
-- ╚══════════════════════════════════════════════════════════╝
...
-- Orden de columnas: Name | Fruit | Color | City/Country | Animal | Object
...
local logLines      = {}
local LogLabel      = nil
local cachedFilaNum = nil   -- cacheamos para no buscar en cada ronda
...
-- ════════════════════════════════════════════════════════════
--  BUSCAR NUESTRO NÚMERO DE FILA
--
--  Estrategia: buscar un TextLabel con nuestro username
--  en la GUI del juego, luego subir por la jerarquía hasta
--  encontrar un frame/objeto que tenga un TextBox_Fila{N}
--  o cuyo nombre contenga Fila{N}.
--
--  Si eso falla: buscar frames cuyo nombre sea nuestro UserId
--  o username.
-- ════════════════════════════════════════════════════════════
local function findMyFilaNum()
    if cachedFilaNum then return cachedFilaNum end

    local root     = getGameGui() or PlayerGui
    local username = LocalPlayer.Name
    local userid   = tostring(LocalPlayer.UserId)

    -- ── Método 1: TextLabel con nuestro nombre ──────────────
    for _, obj in ipairs(root:GetDescendants()) do
        if not obj:IsA("TextLabel") then continue end
        if not obj.Text:find(username, 1, true) then continue end

        -- Subimos hasta 8 niveles buscando TextBox_FilaN o nombre FilaN
        local ancestor = obj.Parent
        for _ = 1, 8 do
            if not ancestor or ancestor == root then break end

            -- ¿El frame se llama algo con FilaN?
            local fn = ancestor.Name:match("[Ff]ila(%d+)")
            if fn then
                cachedFilaNum = tonumber(fn)
                return cachedFilaNum
            end

            -- ¿Algún hijo directo es TextBox_FilaN?
            for _, child in ipairs(ancestor:GetChildren()) do
                if child:IsA("TextBox") then
                    local n = child.Name:match("[Ff]ila(%d+)")
                    if n then
    ...
    -- ── Método 2: Frame con nuestro UserId o nombre ─────────
    for _, obj in ipairs(root:GetDescendants()) do
        if not (obj:IsA("Frame") or obj:IsA("ScrollingFrame")) then continue end
        local oname = obj.Name:lower()
        if not (oname:find(userid,1,true) or oname:find(username:lower(),1,true)) then continue end
    ...
    -- ── Método 3: buscar qué FilaN tiene texto editable Y
    --    nuestro nombre en un Label adyacente ─────────────────
    -- (Agrupamos todos los TextBox_FilaN por número)
    local filaGroups = {}   -- { [1]={boxes}, [2]={boxes}, ... }
    for _, obj in ipairs(root:GetDescendants()) do
        if not obj:IsA("TextBox") then continue end
        if isOurs(obj) then continue end
        local n = obj.Name:match("[Ff]ila(%d+)")
        if n then
            local num = tonumber(n)
            filaGroups[num] = filaGroups[num] or {}
            table.insert(filaGroups[num], obj)
        end
    end

    -- Para cada grupo buscar si hay un Label con nuestro nombre en ancestros comunes
    for filaNum, boxes in pairs(filaGroups) do
        for _, box in ipairs(boxes) do
            local anc = box.Parent
            for _ = 1, 6 do
                if not anc or anc == root then break end
                for _, label in ipairs(anc:GetDescendants()) do
                    if label:IsA("TextLabel") and label.Text:find(username,1,true) then
                        cachedFilaNum = filaNum
                        return cachedFilaNum
                    end
                end
                anc = anc.Parent
...
-- ════════════════════════════════════════════════════════════
--  ENCONTRAR NUESTRAS CAJAS
--  Filtra TextBox_Fila{N} donde N = nuestro FilaNum.
--  Ordena por AbsolutePosition.X → columnas de izq. a der.
--  Mapea al COLUMN_ORDER confirmado.
-- ════════════════════════════════════════════════════════════
local function findMyBoxes()
    local filaNum = findMyFilaNum()
    local root    = getGameGui() or PlayerGui

    if not filaNum then
        -- Fallback: una caja por X único (primer box de cada columna)
        addLog("⚠️ FilaNum no hallado → fallback X-único")
        local all = {}
        for _, obj in ipairs(root:GetDescendants()) do
            if obj:IsA("TextBox") and not isOurs(obj) and obj:IsA("GuiObject") then
                table.insert(all, obj)
            end
        end
        table.sort(all, function(a,b) return a.AbsolutePosition.X < b.AbsolutePosition.X end)
        -- Tomar el primer box por X único (≥10px de diferencia = nueva columna)
        local byCol = {}
        local lastX = -9999
        for _, box in ipairs(all) do
            local x = math.floor(box.AbsolutePosition.X)
            if x - lastX > 10 then
                table.insert(byCol, box)
                lastX = x
            end
        end
        local mapped = {}
        for i, box in ipairs(byCol) do
            local cat = COLUMN_ORDER[i]
            if cat then mapped[cat] = box end
        end
        return mapped, byCol, nil
    end

    -- Filtrar solo TextBox_Fila{N}
    local targetName = "TextBox_Fila"..filaNum
    local myBoxes    = {}
    for _, obj in ipairs(root:GetDescendants()) do
    ...
    -- Ordenar por X → columnas
    table.sort(myBoxes, function(a,b) return a.AbsolutePosition.X < b.AbsolutePosition.X end)
    ...
    return mapped, myBoxes, filaNum
end

-- ════════════════════════════════════════════════════════════
local function isVisible2(obj) return obj:IsA("GuiObject") and obj.Visible end

local function findBastaButton()
    local root = getGameGui() or PlayerGui
    local d    = root:FindFirstChild("BotonBasta", true)
    if d and isVisible2(d) then return d end
    for _, obj in ipairs(root:GetDescendants()) do
        if not isVisible2(obj) then continue end
        if not (obj:IsA("TextButton") or obj:IsA("ImageButton")) then continue end
        local n = obj.Name:lower()
        local t = obj:IsA("TextButton") and obj.Text:lower() or ""
        if n:find("basta") or t:find("basta") or n:find("submit") or n:find("stop") then
            return obj
        end
    end
    return nil
...
local function doSubmit()
    ...
    local mapped, myBoxes, filaNum = findMyBoxes()
    local filled = 0

    if filaNum then
        addLog("🎯 Fila#"..filaNum.." | "..#myBoxes.." cajas")
    end
    ...
    if filled == 0 then addLog("⚠️ 0 cajas llenadas") end
    task.wait(0.15)
...
Frame.Position=UDim2.new(0,58,0.24,0)
...
TitleLbl.BackgroundTransparency=1;TitleLbl.Text="🎲  Basta Bot  v8"
...
local ScanBtn  =mkbtn("🔎  BUSCAR MI FILA (debug)",   130, Color3.fromRGB(55,42,10))
local ManualBtn=mkbtn("📤  LLENAR + BASTA (manual)",  167, Color3.fromRGB(30,75,38))

local logBg=Instance.new("Frame")
logBg.Size=UDim2.new(0,W-14,0,100);logBg.Position=UDim2.new(0,7,0,204)
...
-- SCAN: muestra el FilaNum encontrado y qué cajas tiene
ScanBtn.MouseButton1Click:Connect(function()
    cachedFilaNum = nil   -- resetear caché para re-buscar
    addLog("🔎 Buscando FilaNum...")
    local fn = findMyFilaNum()
    if fn then
        addLog("🎯 Mi Fila#: "..fn)
        local _, myBoxes, _ = findMyBoxes()
        addLog(string.format("Cajas Fila%d: %d", fn, #myBoxes))
        for i, box in ipairs(myBoxes) do
            if i>4 then addLog("..."); break end
            addLog(string.format("  [%d] X=%.0f %s", i, box.AbsolutePosition.X, COLUMN_ORDER[i] or "?"))
        end
    else
        addLog("❌ FilaNum no encontrado")
        addLog("Jugador: "..LocalPlayer.Name)
    end
    local btn=findBastaButton()
    addLog("Botón: "..(btn and btn.Name.."✅" or "❌"))
end)
...
print("[BastaBot] v8 ✅  FilaNum search")
addLog("v8 — Presiona 🔎 en ronda activa")
