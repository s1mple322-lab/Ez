-- ╔══════════════════════════════════════════════════════════╗
-- ║   🎲 BASTA BOT v10  —  Fix: sin Focus, solo .Text        ║
...
local function typeInBox(box, text)
    -- Solo asignar .Text directamente.
    -- CaptureFocus/ReleaseFocus dispara FocusLost del juego que limpia el box.
    box.Text = text
    task.wait(0.03)
end
...
local function doSubmit()
    ...
    -- Espera mayor para que el juego registre los valores
    task.wait(0.6)
...
TitleLbl.BackgroundTransparency=1;TitleLbl.Text="🎲  Basta Bot  v10"
...
print("[BastaBot] v10 ✅  Sin CaptureFocus — fix text clear")
addLog("v10 — 1) SET FILA  2) ACTIVAR")
updateFilaLabel()
