-- ╔══════════════════════════════════════════════════════════╗
-- ║     🎲 BASTA BOT v7  —  TextEditable + BotonBasta       ║
-- ║  Filtra por TextEditable=true → solo nuestra fila       ║
-- ║  Ordena por AbsX → orden de columnas confirmado         ║
-- ║  Botón: BotonBasta (nombre confirmado en scan)          ║
-- ╚══════════════════════════════════════════════════════════╝
...
local function isVisible(obj)
    -- GuiObject tiene .Visible ; ScreenGui/LayerCollector no
...
local function detectGameLetter()
    ...
    local roots = {}
    if gg then
        table.insert(roots, gg)
        local fl = gg:FindFirstChild("FrameLetra", true)
        if fl then table.insert(roots, 1, fl) end
    else
...
-- ════════════════════════════════════════════════════════════
--  ENCONTRAR NUESTRAS CAJAS  ← CLAVE DEL FIX
--
--  De 30 TextBoxes totales (5 jugadores × 6 categorías),
--  solo las de NUESTRA FILA tienen TextEditable = true.
--  Filtramos por eso y ordenamos por AbsolutePosition.X
--  para obtener el orden de columnas: Nombre|Fruta|Color|Ciudad|Animal|Objeto
-- ════════════════════════════════════════════════════════════
local function findMyBoxes()
    ...
    for _, obj in ipairs(root:GetDescendants()) do
        ...
        -- Solo cajas EDITABLES (nuestra fila)
        if not obj.TextEditable then continue end
        table.insert(myBoxes, obj)
    end

    -- Ordenar izquierda → derecha por posición absoluta en pantalla
    table.sort(myBoxes, function(a, b)
        return a.AbsolutePosition.X < b.AbsolutePosition.X
    end)

    -- Mapear al orden de columnas confirmado
    local mapped = {}
...
-- ════════════════════════════════════════════════════════════
--  BOTÓN BASTA  (nombre confirmado: BotonBasta)
-- ════════════════════════════════════════════════════════════
local BASTA_KW = {"botonbasta","basta","stop","submit","enviar","parar","listo"}

local function findBastaButton()
    local gg   = getGameGui()
    local root = gg or PlayerGui

    -- Búsqueda directa por nombre exacto (más rápida)
    local direct = root:FindFirstChild("BotonBasta", true)
    if direct and isVisible(direct) then return direct end
...
-- ════════════════════════════════════════════════════════════
--  ESCRIBIR EN TEXTBOX (simula input real)
-- ════════════════════════════════════════════════════════════
...
local function doSubmit()
    ...
    -- 1. Encontrar y llenar NUESTRAS cajas (TextEditable=true)
    local mapped, myBoxes = findMyBoxes()
    local filled = 0

    if #myBoxes == 0 then
        addLog("⚠️ 0 cajas editables (¿ronda activa?)")
    else
        addLog(string.format("✅ %d cajas editables", #myBoxes))
    ...
    -- 3. Backup si no hubo nada de UI
...
TitleLbl.BackgroundTransparency=1;TitleLbl.Text="🎲  Basta Bot  v7"
...
local ScanBtn   = mkbtn("🔎  ESCANEAR (debug)",         130, Color3.fromRGB(55,42,10))
...
-- SCAN: muestra cuántas cajas EDITABLES hay (nuestra fila)
ScanBtn.MouseButton1Click:Connect(function()
    local gg,gname=getGameGui()
    addLog("GUI: "..(gname or "❌ no encontrado"))
    local mapped, myBoxes = findMyBoxes()
    addLog(string.format("Cajas edit: %d", #myBoxes))
    for i, box in ipairs(myBoxes) do
        if i>4 then addLog("..."); break end
        addLog(string.format(" [%d] %s X=%.0f", i, box.Name, box.AbsolutePosition.X))
    end
    local nMapped=0; for _ in pairs(mapped) do nMapped=nMapped+1 end
    addLog(string.format("Categ: %d | Botón: %s", nMapped,
        findBastaButton() and "BotonBasta✅" or "❌"))
end)
...
print("[BastaBot] v7 ✅  TextEditable+BotonBasta")
addLog("v7 OK — Activa y espera la ronda")
