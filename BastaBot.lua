...
local function sendAnswers()
    -- 1. Verificar que el remote existe
    if not SubmitAnswers then
        addLog("❌ SubmitAnswers = nil (esperando...)")
        -- Intento de rescate en tiempo real
        local be = RepStorage:FindFirstChild("BastaEvents")
        if be then
            SubmitAnswers = be:FindFirstChild("SubmitAnswers")
        end
        if not SubmitAnswers then
    ...
    addLog(string.format("📤 Enviando [%s] %s/%s/%s", currentLetter,
    ...
    if ok then
        addLog("✅ FireServer OK")
    else
        addLog("❌ Error: " .. tostring(err):sub(1,45))
    end
end
...
local ToggleBot = btn("⚡  ACTIVAR  AUTO-BOT",  93,  Color3.fromRGB(46,46,92))
local ManualBtn = btn("📤  ENVIAR AHORA (manual)", 131, Color3.fromRGB(35,82,44))
local TestBtn   = btn("🔍  TEST CONEXIÓN REMOTE",  169, Color3.fromRGB(75,55,20))

-- ── Mini-log (4 líneas) ───────────────────────
local logBg = Instance.new("Frame")
logBg.Size=UDim2.new(0,W-14,0,60); logBg.Position=UDim2.new(0,7,0,207)
...
ToggleBot.MouseButton1Click:Connect(function()
    ...
    if isActive then
        ...
        if not ForceSubmitEvent then
            addLog("❌ ForceSubmitAnswers nil; solo manual")
        else
